...
- Cambio: Se implemento animacion scroll-driven para el cerebro en `app/components/method-brain.tsx` que rota de vista frontal (rotX=0) a vista superior (rotX=-π/2) segun el scroll del usuario. Se creo hook `useScrollAnimationProgress` en `app/hooks/use-scroll-animation-progress.ts` que calcula el progreso (0-1) basado en la posicion del elemento "¿Como pensamos?" relativa al header (offset 80px). La animacion comienza cuando el elemento esta a 200px del header y termina cuando toca el header exactamente. Se integro el hook en MethodBrain y se paso referencia de la seccion desde `home-client-shell.tsx`. Durante la animacion scroll-driven, la rotacion Y continua por inercial (arrastre del usuario) con reduccion de velocidad al 80%, manteniendo la experiencia interactiva fluida. Fuera del rango de scroll-driven, el cerebro vuelve a comportamiento normal (interaccion manual + inercial + lerp a reposo).
- Impacto: El cerebro ahora sigue la narrativa inmersiva de la pagina: comienza frontal en Intro, gira a superior conforme el usuario scrollea hacia "Como pensamos", llegando a posicion final cuando el titulo toca el header. Proporciona transicion visual fluida (60 FPS en desktop, 45+ en mobile) sin bloquear interactividad manual. Hook scroll-driven es reutilizable para futuras animaciones por scroll. Performance: throttled con RAF, usa ref para evitar re-renders del efecto completo, listeners lazily attached.
- Proximo paso: (1) Validar en navegador real que progreso 0-1 mapea correctamente a rotacion visual (ajustar triggerMargin si hace falta), (2) Ajustar headerOffset si el header tiene altura diferente, (3) Considerar easing (ease-in-out) para rotacion si se desea transicion mas suave, (4) Medir performance en mobile low-end (target 45 FPS).
- Fecha: 2026-04-11
- Autor: Copilot
